...
func (g *Genlua) genLocal(file string) {
	...
	if ok {
		...
		u, err := user.Current()
		if err == nil {
			buf.WriteString("--Generated by sxgen, version=" + Version + ". By " + u.Username + " at time " + time.Now().Format("2006-01-02 15:04:05") + "\n")
		...
		for index, cell := range row.Cells {
			...
			if isBaseType(tname) {
				...
				if !isBaseType(tk) && tk != "string" {
					Error("lua: Dictionary只支持key/value都是string或基本数据类型 : " + tname)
				}
				value = ""
				var curfname string
				var lastAnno string
